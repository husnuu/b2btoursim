import { VILLA_STATUS_META, type VillaStatus } from "@/lib/villa-status";
...
export function VillaStatusBadge({ status, className = "" }: { status: VillaStatus; className?: string }) {
  const meta = VILLA_STATUS_META[status];
  return (
    <span
