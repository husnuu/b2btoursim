import { BOAT_STATUS_META, type BoatStatus } from "@/lib/boat-status";
...
export function BoatStatusBadge({ status, className = "" }: { status: BoatStatus; className?: string }) {
  const meta = BOAT_STATUS_META[status];
  return (
    <span
